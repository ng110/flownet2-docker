...
nvidia-docker run --rm --volume "${PWD}:/input-output:rw" \
    --workdir "/flownet2/flownet2/scripts" -it "flownet2" \
    /bin/bash -c "cd ..; source set-env.sh; cd -; python run-flownet-docker.py --verbose --gpu 0 ${WEIGHTS} ${DEPLOYPROTO} ${FIRST_INPUT} ${SECOND_INPUT} ${OUTPUT}"
else